...
// WHAT IS GOING ON HERE?
    char fname[256];
    snprintf(fname,256,"mres.%d",tsrc);
    fp = Fopen(fn, "a+");
    if(!fp) ERR.FileW("","run_mres()",fname);
    for(unsigned t = 0; t < t_size_glb; ++t) {
        Fprintf(fp, "%3u %3u %17.10e %17.10e %17.10e %17.10e\n", tsrc, t,
                pion[t].real(), pion[t].imag(),
                j5_q[t].real(), j5_q[t].imag());
...
#pragma omp for
        for(int i = 0; i < lcl_vol; ++i) {
            ...
            a5_tmp[t_glb] += 2.0 * mass * Trace(p[0], p[1]);
            a5_tmp[t_glb] += 2.0 * Trace(q[0], q[1]);
        } // sites
#pragma omp critical
        for(int t = 0; t < t_size_glb; ++t) {
