...
protected:
  ...
  int CheckSCfile( char *infile){
    ...
    if(FILE* ftmp=Fopen(file,"r")) {
      ...
      VRB.Result(cname,fname, "propagator file %s with  divided  spin/color sources found\n", file);
    } else {
      sc_part_file_exist =0;
      VRB.Result(cname,fname, "propagator file %s with  divided  spin/color sources NOT found\n ", file);
    }
...
public:
  ...
  void CG(std::vector<int> &spn, std::vector<int> &clr, 
	std::vector<FermionVectorTp*> source , std::vector<FermionVectorTp*> sol, 
	std::vector<FermionVectorTp*> midsol, std::vector<int>&iter, std::vector<Float>&true_res);
...
#define   WallWallProp WallSinkProp
  ...
  void gf_vec(Lattice & lat, Vector* out, Vector* in);
};
...
public:
    ...
    QPropWZ3BWallSrc(Lattice& lat, CommonArg* c_arg);
...
public:
  ...
  QPropWRand(Lattice& lat, QPropWArg* , QPropWRand*, CommonArg*);
  QPropWRand(Lattice& lat, QPropWArg* , QPropWS*, CommonArg*);
...
public:
  QPropWRandVolSrc(Lattice& lat, CommonArg* c_arg);
	//added run_flag. Only do CG if it's true.
  QPropWRandVolSrc(Lattice& lat, QPropWArg* , QPropWRandArg *, CommonArg*, bool run_flag = true);
	//Use the random numbers source from another QPropW
  QPropWRandVolSrc(Lattice& lat, QPropWArg* , QPropWRand *, CommonArg*, bool run_flag = true);
	//Use the random numbers source from another QPropWS
  QPropWRandVolSrc(Lattice& lat, QPropWArg* , QPropWS *, CommonArg*, bool run_flag = true);
   
  void SetSource(FermionVectorTp& src, int spin, int color);
  SourceType SrcType() { return RANDVOLUME; }
};

class QPropWA2ALoopLow : public QPropW {
        public:
                QPropWA2ALoopLow(Lattice & lat, QPropWArg *arg, CommonArg *c_arg);
};
