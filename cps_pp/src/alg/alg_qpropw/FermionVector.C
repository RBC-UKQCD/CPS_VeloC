...
static inline void compute_coord(int x[4], const int lcl[4], const int low[4], int i)
{
...
void FermionVectorTp::SetZ3BWall(int color, int spin, int t, const int size[4],
                                 const std::vector<Rcomplex> &rand_num)
{
    const char *fname = "SetZ3BWall()";
    int t_offset = t % size[3]; // size[3] used as a period
    ...
    VRB.Result(cname,fname,"size=%d %d %d %d t_offset=%d\n",size[0],size[1],size[2],size[3],t_offset);
    VRB.Result(cname,fname,"lcl=%d %d %d %d\n",lcl[0],lcl[1],lcl[2],lcl[3]);
    printf("Node %d shift=%d %d %d %d\n",UniqueID(),shift[0],shift[1],shift[2],shift[3]);fflush(stdout);
...
// Changed from Hantao's version to avoid putting sources in remainder portion of the 3D slices when size[] does not divide glb[]
    ...
    VRB.Result(cname,fname,"rand_grid=%d %d %d %d\n",rand_grid[0],rand_grid[1],rand_grid[2],rand_grid[3]);
...
//        if(glb_x[3] != t) continue;
    bool inbox=true;
    for(int mu = 0; mu < 3; ++mu) {
        if(glb_x[mu] >= (size[mu]*rand_grid[mu]) ) inbox=false;
    }

    if( (glb_x[3]-t_offset+size[3]) % size[3]) inbox=false;
if (inbox){
        int id = 0;
        for(int k = 0; k < 4; ++k) {
            id = id * rand_grid[k] + glb_x[k] / size[k];
        }
//	if((!glb_x[0])&&(!glb_x[1])&&(!glb_x[2])){
	if(  !(glb_x[0] % size[0]) &&
	!(glb_x[1] % size[1]) &&
	!(glb_x[2] % size[2]) ) {
	printf("Node %d: i=%d glb_x=%d %d %d %d id=%d %e %e \n",UniqueID(),i, glb_x[0],glb_x[1],glb_x[2],glb_x[3], id,std::real(rand_num[id]), std::imag(rand_num[id]));
        fflush(stdout);
        }
#if 1
	if (id>=rand_num.size()) ERR.General(cname,fname,"i=%d glb_x=%d %d %d %d id(%d)>=rand_num.size(%d)!\n",i, glb_x[0],glb_x[1],glb_x[2],glb_x[3], id,rand_num.size());
