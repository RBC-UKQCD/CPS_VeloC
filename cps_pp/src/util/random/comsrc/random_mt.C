...
#if 0 
 int veloc_id= -1;
 veloc_id= CheckPinter::VeloCCounter();
// veloc_cps->mem_protect (  veloc_id  , p, m_size, d_size );
// veloc_cps->mem_protect (veloc_id  , veloc::cereal::serializer(cpsran),veloc::cereal::deserializer(cpsran));
 veloc_cps->mem_protect (veloc_id  , veloc::bitsery::serializer(cpsran),veloc::bitsery::deserializer(cpsran));
 if (veloc_id>-1) CheckPointer::md_veloc_all.insert(veloc_id);
#endif
...
// if(!cpsran) ERR.Pointer(cname, fname, "cpsran"); 
  ...
  VRB.Result(cname,fname,"base_seed=%d\n",base_seed);
//check if base_seed is uniform
  uint32_t ret = base_seed;
  QMP_broadcast(&ret,sizeof(uint32_t));
  if ( ret != base_seed)
    ERR.General(cname,fname,"base_seed(%d) is different from node zero(%d)\n",base_seed,ret);
