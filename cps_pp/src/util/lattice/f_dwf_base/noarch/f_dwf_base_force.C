...
{
  if (GJP.Gparity ())
    return EvolveMomFforceGparity (mom, chi, mass, dt);
  ...
  if (GJP.Gparity1fX ()) {
    gparity_1f_mombuf =
      (Matrix *) fmalloc (cname, fname, "gparity_1f_mombuf",
			  4 * GJP.VolNodeSites () * sizeof (Matrix));
    for (int i = 0; i < 4 * GJP.VolNodeSites (); i++)
      gparity_1f_mombuf[i].ZeroMatrix ();
  }
	...
	for (y = 0; y < ly; y++) {
	  for (x = 0; x < lx; x++) {
	    ...
	    sproj_tr[mu + 4] ((IFloat *) & tmp_mat2,
			      ...
			      ls, vec_plus_mu_stride,
			      f_size_4d - f_site_size_4d);
...
//        if(!UniqueID())printf("%s::%s:GJP.Snodes()=%d\n",cname,fname,GJP.Snodes()); 
	      glb_sum_multi_dir ((Float *) & tmp_mat1, 4,
				 sizeof (Matrix) / sizeof (IFloat));
	    }
	    ...
	    *(mom + gauge_offset) += tmp_mat2;
	    ...
	    if (GJP.Gparity1fX ())
	      *(gparity_1f_mombuf + gauge_offset) = tmp_mat2;
  ...
  if (GJP.Gparity1fX ()) {
    ...
    Matrix *buf2 =
      (Matrix *) fmalloc (cname, fname, "buf2", momsz * sizeof (Float));
...
#pragma omp parallel for default(shared) private(mu)
  ...
  if (GJP.Gparity1fX ())
    ffree (gparity_1f_mombuf, cname, fname, "gparity_1f_mombuf");
	  ...
	  for (x = 0; x < lx; x++) {
		...
		for (s = 0; s < ls; s++) {
		  ...
		  getPlusData ((Float *) site_v1 + s * f_site_size_4d,
			       (Float *) v1 + vec_plus_mu_offset +
			       s * f_size_4d, f_site_size_4d, mu);
		  getPlusData ((Float *) site_v2 + s * f_site_size_4d,
			       (Float *) v2 + vec_plus_mu_offset +
			       s * f_size_4d, f_site_size_4d, mu);
		}		// end for s
					   ...
					   Float mass, Float dt)
{
  ...
  if (!GJP.Gparity ())
    ERR.General (cname, fname,
		 "Function executed when G-parity BCs not in use\n");
	...
	for (y = 0; y < ly; y++) {
	  for (x = 0; x < lx; x++) {
	    ...
	    /* CK: Replaced nasty 300000 line switch statement with the following: */
	    {
	      ...
	      vec_plus_mu_offset *=
		pos_p_mu[0] + lx * (pos_p_mu[1] +
				    ly * (pos_p_mu[2] + lz * pos_p_mu[3]));
	    }
				    ...
				    Float mass, Float dt)
{
  ...
  if (GJP.Gparity ()) {
    ERR.General (cname, fname,
		 "Not implemented for G-parity boundary conditions");
  }
  // allocate space for two CANONICAL fermion fields
