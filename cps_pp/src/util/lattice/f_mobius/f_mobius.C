		     ...
		     PreserveType prs_f_in, int dminus)
{
  ...
  size_t f_size = GJP.VolNodeSites() * GJP.SnodeSites() * 2 * Colors() * SpinComponents();
  assert(this->full_size == f_size);
  if(prs_f_in==PRESERVE_YES){ 
    temp = (Vector *) smalloc(cname,fname, "temp",f_size * sizeof(Float));
    moveFloat((IFloat*)temp,(IFloat*)f_in, f_size);
  }

  DiracOpMobius dop(*this, f_out, f_in, cg_arg, cnv_frm);

  if (dminus){
    VRB.Debug(cname,fname,"Dminus applied\n");
    Float norm = f_in->NormSqGlbSum(f_size);
    dminus_in = (Vector *) smalloc(cname,fname, "temp",f_size * sizeof(Float));
    dop.Dminus(dminus_in,f_in);
    moveFloat((IFloat*)f_in, (IFloat*)dminus_in, f_size);
    VRB.Result(cname,fname,"f_in=%g dminus=%g\n",norm, f_in->NormSqGlbSum(f_size));
    sfree(cname, fname,  "dminus_in",  dminus_in);
  }

//  Float inv_time =-dclock();
  timer_cg.start();
//  iter = dop.MatInv(true_res, PRESERVE_NO);
  iter = dop.MatInv(true_res, prs_f_in);
  timer_cg.stop();
//  inv_time +=dclock();
//  print_time(fname,"MatInv()",inv_time);
  if(prs_f_in==PRESERVE_YES){
    moveFloat((IFloat*)f_in,(IFloat*)temp, f_size);


    // TIZB check
if (0){
    Float norm;
    norm = f_out->NormSqGlbSum(f_size);
    VRB.Result(cname,fname,"f_mobius  Norm out %.14e\n",norm);
    norm = f_in->NormSqGlbSum(f_size);
    VRB.Result(cname,fname,"f_mobius Norm in %.14e\n",norm);
    dop.Mat(temp,f_out);  
    norm = temp->NormSqGlbSum(f_size);
...
#if 0
  f_out -> VecTimesEquFloat(1.0 / ( GJP.Mobius_b() * ( 4.0 - GJP.DwfHeight() ) + 1.0 ), f_size);
#endif
  Float *f_out_p=(Float*)f_out;
  Float *f_in_p=(Float*)f_in;
  VRB.Result(cname,fname,"f_out=%e (%e %e) f_in=%e (%e %e) cg_arg=%p true_res=%p cnv_frm=%d prs_f_in=%d dminus=%d\n",
f_out->NormSqGlbSum(full_size),*f_out_p,*(f_out_p+1),
f_in->NormSqGlbSum(full_size),*f_in_p,*(f_in_p+1),
cg_arg,true_res,cnv_frm,prs_f_in,dminus);
  //
...
void Fmobius::FmatInv(std::vector<Vector *>f_out, std::vector<Vector *>f_in, 
		     CgArg *cg_arg, 
...
{
//  int iter;
  const char *fname = "FmatInv(**V,**V,F*,CnvFrmType)";
  VRB.Func(cname,fname);
  int len = f_out.size();
  ...
  std::vector<Vector *>temp(len,NULL);
  Vector *dminus_in;
//  size_t f_size = GJP.VolNodeSites() * GJP.SnodeSites() * 2 * Colors() * SpinComponents();
//  assert(this->full_size == f_size);
    size_t f_size=this->full_size ;
  
for(int i=0;i<len;i++)
  if(prs_f_in==PRESERVE_YES){ 
    temp[i]=( (Vector *) smalloc(cname,fname, "temp",f_size * sizeof(Float)) );
    moveFloat((IFloat*)temp[i],(IFloat*)f_in[i], f_size);
  }

  if (cnv_frm == CNV_FRM_YES)
for(int i=0;i<len;i++){
      Fconvert(f_in[i], DWF_4D_EOPREC_EE, CANONICAL);
      Fconvert(f_out[i], DWF_4D_EOPREC_EE, CANONICAL);
}

  DiracOpMobius dop(*this, f_out[0], f_in[0], cg_arg, CNV_FRM_NO);

  if (dminus){
  VRB.Flow(cname,fname,"Dminus applied\n");
    dminus_in = (Vector *) smalloc(cname,fname, "dminus_in",f_size * sizeof(Float));
for(int i=0;i<len;i++){
//    dminus_in->VecZero(f_size);
    Float norm = f_in[i]->NormSqGlbSum(f_size);
    dop.Dminus(dminus_in,f_in[i]);
    moveFloat((IFloat*)f_in[i], (IFloat*)dminus_in, f_size);
    VRB.Result(cname,fname,"f_in=%g dminus=%g\n",norm, f_in[i]->NormSqGlbSum(f_size));
}
    sfree(cname, fname,  "dminus_in",  dminus_in);
  }

  timer_cg.start();
  VRB.Result(cname,fname,"len=%d f_out=%d f_in=%d true_res=%d iters=%d\n",len,f_out.size(),f_in.size(),true_res.size(),iters.size());
//  iter = dop.MatInv(true_res, prs_f_in);
  Float *res_p = true_res.data();
#if 0
for(int i=0;i<len;i++){
  VRB.Result(cname,fname,"len=%d f_out=%p f_in=%p true_res=%p\n",i,f_out[i],f_in[i],res_p+i);
  iters[i]=dop.MatInv(f_out[i],f_in[i],res_p+i, prs_f_in);
}
#else
  dop.MatInv(f_out,f_in, true_res,iters);
#endif
  timer_cg.stop();
for(int i=0;i<len;i++){
      Fconvert(f_in[i], CANONICAL, DWF_4D_EOPREC_EE);
      Fconvert(f_out[i], CANONICAL, DWF_4D_EOPREC_EE);
}
for(int i=0;i<len;i++){
  if(prs_f_in==PRESERVE_YES){
    moveFloat((IFloat*)f_in[i],(IFloat*)temp[i], f_size);
    sfree(cname, fname,  "temp",  temp[i]);
  }
  VRB.Result(cname,fname,"%d: f_in=%g f_out=%g\n",i, f_in[i]->NormSqGlbSum(f_size),f_out[i]->NormSqGlbSum(f_size));
}
  // DJM: CPS and QUDA have different Dslash normalizations --- correct this.
#if 1
for(int i=0;i<len;i++){
//  f_out[i] -> VecTimesEquFloat(1.0 / ( GJP.Mobius_b() * ( 4.0 - GJP.DwfHeight() ) + 1.0 ), f_size);
  VRB.Result(cname,fname,"f_out=%e f_in=%e cg_arg=%p true_res=%e cnv_frm=%d prs_f_in=%d dminus=%d\n",f_out[i]->NormSqGlbSum(full_size),f_in[i]->NormSqGlbSum(full_size),cg_arg,true_res[i],cnv_frm,prs_f_in,dminus);
}
#endif
  //
...
{
  int iter;
  const char *fname = "FeigSolv(V*,F*,LanczosArg*,CnvFrmType)";
  VRB.Func(cname,fname);
  CgArg cg_arg;
  cg_arg.mass = eig_arg->mass;
...
void Fmobius::Fdslash(Vector *f_out, Vector *f_in, CgArg *cg_arg,
  ...
  const char *fname = "Fdslash(V*,V*,CgArg*,CnvFrmType,i)";
  static Timer timer (cname,fname);
  timer.start();
  DiracOpMobius dop(*this, f_out, f_in, cg_arg, cnv_frm);
...
{
  int iter;
  const char *fname = "FmatEvlInv(CgArg*,V*,V*,F*,CnvFrmType)";
  VRB.Func(cname,fname);
  static Timer timer (cname,fname);
  timer.start();
...
{
  int iter;
  const char *fname = "FmatEvlMInv(V**,V*,.....)";
  VRB.Func(cname,fname);
//  Float dtime = -dclock();
  static Timer timer (cname,fname);
  timer.start();
...
//    long f_size = (long)SPINOR_SIZE * GJP.VolNodeSites() * Fmobius::bfm_args[current_arg_idx].Ls;
  ...
  if(GJP.EOFA())
  {
    ...
    #ifdef PROFILE
    dtime += dclock();
    print_flops(fname, "g5R5+CopyVec()", 0, dtime);
    dtime -= dclock();
    #endif
