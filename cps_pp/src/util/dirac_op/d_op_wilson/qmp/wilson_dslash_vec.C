                        ...
                        int vec_len, unsigned long vec_offset)
{
  ...
  static char *cname = "";
  static char *fname = "wilson_dslash_vec";
  ...
  if (GJP.Gparity ()) ERR.General(cname,fname,"GJP.Gparity on somehow");
  VRB.Debug(cname,fname,"vec_len=%d vec_offset=%d chi=%p u=%p psi=%p\n",cname,fname,UniqueID(), vec_len,vec_offset,chi_p_f,u_p_f,psi_p_f);
...
//      fflush(stdout);
  ...
  VRB.Debug(cname,fname,"n_dir=%d multiple=%p\n",n_dir,multiple);
...
//                  Printf ("getMinusData((IFloat *)fbuf, (IFloat *)tmp%d, SPINOR_SIZE, %d);\n", dir + 1, mu);
...
//  if(!UniqueID()) printf("%s:%s: Node %d: n_dir=%d multiple=%p\n",cname,fname,UniqueID(), n_dir,multiple);
...
//	printf("%s:%s: Node %d: i_mu=%d chi=%p u=%p psi=%p\n",cname,fname,UniqueID(),i_mu,chi,u,psi);
        ...
        for (int vec_ind = 0; vec_ind < vec_len; vec_ind++) {
//	  printf("%s:%s: Node %d: i_mu=%d chi=%p u=%p psi=%p\n",cname,fname,UniqueID(),i_mu,chi,u,psi);fflush(stdout);
          for (s = 0; s < 4; s++)
            for (c = 0; c < 3; c++)
              for (r = 0; r < 2; r++)
