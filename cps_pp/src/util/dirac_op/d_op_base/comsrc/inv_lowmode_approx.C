...
//    printf("cache_name=%s neig=%d ecache=%p\n",fname_eig_root_bc,neig,ecache);
    ...
    VRB.Result (cname, fname, "returned %s %d %d neig=%d\n", ecache->Name (),
                ecache->Neig (), ecache->NeigCoarse (),neig);
//  exit(-42);
...
//------------------------------------------------------------------
 ...
 VRB.Result(cname,fname,"ecache=%p neig=%d neig_b=%d neig=%d\n",ecache,ecache->Neig(),ecache->NeigCoarse(),neig);
  if(  ecache->Neig() <neig) {
