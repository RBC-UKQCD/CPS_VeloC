...
//      uint32_t recv[n_elem];
      std::vector<uint32_t> recv(n_elem);
      std::vector<uint32_t> send_temp(n_elem);
      for(int i=0;i<n_elem;i++) send_temp[i] = send[i];
      if(!UniqueID()) printf("glb_sum(%p %d)\n",send, send_temp.data(),n_elem);
      int ret= MPI_Allreduce ((uint32_t *) send_temp.data(), recv.data(), n_elem, MPI_UNSIGNED,
			    MPI_SUM, MPI_COMM_WORLD);
      for(int i=0;i<n_elem;i++) send[i] = recv[i];
//	memcpy(send,recv.data(),n_elem*sizeof(uint32_t));
	return ret;
    }
