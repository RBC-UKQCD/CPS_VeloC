...
int main(int argc, char *argv[]){
  ...
  if ( argc!=11 ) { 
    printf("Args:\t do_arg.vml doext_arg.vml hmc_arg.vml evo_arg.vml \n");
    printf("\t gauge_arg.vml frm_arg.vml \n");
    printf("\t ab1_arg.vml ab2_arg.vml quo_arg.vml \n");
    printf("\t current_dir \n");
    exit(-1);
  }
  ...
  if ( !doext_arg.Decode(argv[2],"doext_arg") ) {
    ...
    doext_arg.Encode("bum_arg","bum_arg"); printf("Bum doext_arg\n"); exit(-1); }
  doext_arg.Encode("doext_arg.dat", "doext_arg");
  if ( !hmc_arg.Decode(argv[3],"hmc_arg")){printf("Bum hmc_arg\n"); exit(-1);}
  hmc_arg.Encode("hmc_arg.dat","hmc_arg");
  if ( !evo_arg.Decode(argv[4],"evo_arg")){printf("Bum evo_arg\n"); exit(-1);}
  ...
  if ( !frm_arg.Decode(argv[6],"frm_arg")){
  ...
  if ( !quo_arg.Decode(argv[9],"quo_arg")){printf("Bum quo_arg\n"); exit(-1);}
  ...
  // must precede all AlgAction allocations
   ...
   //AlgActionFermion dwf(mom, frm_arg);
   ...
   //  top-level integrator:  ab4_arg
    ...
    //AlgIntAB &ab2 = AlgIntAB::Create(ab1, dwf,  ab2_arg);
  ...
  for(int conf=0; conf< evo_arg.gauge_configurations; conf ++ ) {
	...
	//!< Run hybrid Monte Carlo
        ...
        if ( (traj > 19) && (hmc_arg_pass.metropolis == METROPOLIS_NO)) { 
           printf("Switching to ACCECPT/REJECT at traj %d \n",traj);
    ...
    if ((w_int>0) && (traj%w_int==0)){
      ...
      dtime = -dclock();
      printf("Measuring hadron masses\n");
